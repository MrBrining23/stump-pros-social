export default async function handler(req, res) {
  ...
  const { action, token, text, channelId, dueAt, imageUrl, thumbUrl, service } = req.body;
  ...
  try {
    ...
    if (action === 'create') {
      let assetsBlock = '';
      // Use thumbUrl (direct image from Google) instead of Drive download URL
      const finalImageUrl = thumbUrl || imageUrl;
      if (finalImageUrl) {
        const escapedUrl = finalImageUrl.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
