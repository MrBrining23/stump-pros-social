  ...
  const pushToBuffer = async () => {
    ...
    for (let i = 0; i < ap.length; i++) {
      ...
      try {
        ...
        if (post.photo && post.photo.driveId) {
          // Upload full-res image to Cloudinary, get direct URL for Buffer
          setPushRes("Uploading photo " + (i + 1) + "/" + ap.length + "...");
          try {
            const imgResp = await fetch("/api/image", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ driveFileId: post.photo.driveId, driveApiKey: driveConfig.apiKey }),
            });
            const imgData = await imgResp.json();
            if (imgData.url) {
              body.imageUrl = imgData.url;
            } else {
              // Fall back to thumbnail if Cloudinary fails
              body.thumbUrl = post.photo.thumb ? post.photo.thumb.replace(/=s\d+/, "=s1600") : null;
            }
          } catch (e) {
